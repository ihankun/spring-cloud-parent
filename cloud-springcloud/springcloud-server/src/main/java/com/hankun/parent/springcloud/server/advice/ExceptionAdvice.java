...
public class ExceptionAdvice {
    ...
    private void clearContext(HttpServletRequest request) {
        ...
        if (!StringUtils.isEmpty(xid)) {
            ...
            log.info("KunHandlerExceptionResolvers.doResolveException.clear.seata,xid={}", xid);
        }
        ...
        if (!StringUtils.isEmpty(domain)) {
            DomainContext.clear();
            log.info("KunHandlerExceptionResolvers.doResolveException.clear.domain,domain={}", domain);
        }

        String timeStr = BusinessStartTimeContext.getTimeStr();
        if (!StringUtils.isEmpty(timeStr)) {
            BusinessStartTimeContext.clear();
            log.info("KunHandlerExceptionResolvers.doResolveException.clear.time,timeStr={}", timeStr);
        }

        String traceId = TraceLogContext.get();
        if (!StringUtils.isEmpty(traceId)) {
            TraceLogContext.reset();
            log.info("KunHandlerExceptionResolvers.doResolveException.clear.trace,traceId={}", traceId);
        }

        String gray = GrayContext.get();
        if (!StringUtils.isEmpty(gray)) {
            GrayContext.clear();
            log.info("KunHandlerExceptionResolvers.doResolveException.clear.gray,gray={}", gray);
        }
    }
