package com.hankun.parent.commons.cron;
...
public class CronGenerator {
     ...
     * @param includeFirst 是否包含首次执行时间
     * @return the next value matching the pattern
     */
    public Date next(Date date, boolean includeFirst) {
		/*
		The plan:
		...
		*/
        ...
        if (calendar.getTimeInMillis() == originalTimestamp && lastTime != null && includeFirst) {
            // We arrived at the original timestamp - round up to the next whole second and try again...
            calendar.add(Calendar.SECOND, 1);
            doNext(calendar, calendar.get(Calendar.YEAR));
        }
        if (calendar.getTimeInMillis() == originalTimestamp && !includeFirst) {
            calendar.add(Calendar.SECOND, 1);
            doNext(calendar, calendar.get(Calendar.YEAR));
        }
    ...
    private void doNext(Calendar calendar, int dot) {
        ...
        if (month != updateMonth) {
            int length = 4;
            if (calendar.get(Calendar.YEAR) - dot > length) {
...
//        setDaysOfMonth(this.daysOfMonth, fields[3]);
        ...
        for (String field : fields) {
            if (!field.contains("/")) {
                ...
                if ("*".equals(split[0])) {
                    ...
                    bits.set(calendar.get(Calendar.DAY_OF_MONTH));
    ...
    private int[] getRange(String field, int min, int max) {
        ...
        if (!field.contains(contains)) {
            ...
            String delimiter = "-";
            String[] split = StringUtils.delimitedListToStringArray(field, delimiter);
            int length = 2;
    ...
    public static void main(String[] args) {
        ...
        CronGenerator generator = new CronGenerator(cron);

        Date date = DateUtil.dateStrToDate("2020-08-03 08:00:00");

        int length = 20;
        for (int i = 0; i < length; i++) {
            Date next = generator.next(date, false);
            date = next;
            System.out.println(DateUtil.getDateStr(next));
        }
